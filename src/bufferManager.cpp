...
Page BufferManager::getPartition(string tableName, int pageIndex,int colCount, int rowCount)
{
    logger.log("BufferManager::getPage");
    string pageName = "../data/temp/"+tableName + "_Page" + to_string(pageIndex);
    if (this->inPool(pageName))
        ...
        return this->insertPartitionIntoPool(tableName, pageIndex,colCount,rowCount);
}

Page BufferManager::insertPartitionIntoPool(string tableName, int pageIndex,int colCount,int rowCount)
{
    //cout<<" Page access for "<<tableName<<" "<<pageIndex<<endl;
    logger.log("BufferManager::insertIntoPool");
    Page page(tableName, pageIndex,colCount,rowCount);
    if (this->pages.size() >= BLOCK_COUNT)
        pages.pop_front();
    pages.push_back(page);
    return page;
